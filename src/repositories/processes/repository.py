...
def fetch_by_origin():
    sql = """
        SELECT COUNT(*) AS total,
               ...
               TRY_CONVERT(date, NULLIF(LTRIM(RTRIM(T03.DAT_INSTANCIA)), '')) AS data_instancia
        ...
        GROUP BY T02.DES_ATRIBUTO, TRY_CONVERT(date, NULLIF(LTRIM(RTRIM(T03.DAT_INSTANCIA)), ''))
        ORDER BY total DESC
    """
    return run_query(sql)
...
def fetch_by_matter():
    sql = """
        ...
        LEFT JOIN INS_INSTANCIA_VALENCA T03 
            ON T01.ISN_PROCESSO = T03.ISN_PROCESSO
        GROUP BY T02.NOM_MATERIA, TRY_CONVERT(date, NULLIF(LTRIM(RTRIM(T03.DAT_INSTANCIA)), ''))
...
def fetch_by_group():
    sql = """
        ...
        LEFT JOIN INS_INSTANCIA_VALENCA T03 
            ON T01.ISN_PROCESSO = T03.ISN_PROCESSO
        GROUP BY T02.DSC_GRUPO_PROCESSO, TRY_CONVERT(date, NULLIF(LTRIM(RTRIM(T03.DAT_INSTANCIA)), ''))
...
def fetch_by_organization():
    sql = """
        SELECT COUNT(*) AS total,
               ...
               TRY_CONVERT(date, NULLIF(LTRIM(RTRIM(T02.DAT_INSTANCIA)), '')) AS data_instancia
        FROM PRO_PROCESSO_VALENCA T01
        ...
        GROUP BY T03.DSC_ORGAO, TRY_CONVERT(date, NULLIF(LTRIM(RTRIM(T02.DAT_INSTANCIA)), ''))
